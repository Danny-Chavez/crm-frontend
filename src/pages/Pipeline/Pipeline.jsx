...
const handleCreate = async () => {
  ...
  try {
    // 1️⃣ Crear oportunidad
    const res = await api.post("/oportunidades", newOpportunity);
    const nuevaOportunidad = res.data;

    // 2️⃣ Registrar actividad en Timeline
    try {
      await api.post(`/oportunidades/${nuevaOportunidad.id}/actividad`, {
        tipo: "creación",
...
const handleDragEnd = async (event) => {
 ...
 const stageDestino = stages.find((s) => s.id === newStageId);
  ...
  try {
    // Primero intentamos actualizar en backend
    await api.patch(`/oportunidades/${opportunityId}/stage`, {
      stage: newStageId,
      usuario: localStorage.getItem("usuario_nombre"),
    });

    // Si el backend acepta → recién ahí movemos la tarjeta
    setOpportunities((prev) =>
      prev.map((o) =>
    ...
    // Si el backend devuelve validación 400 → mostrar errores específicos
...
return (
  <div className="flex flex-col gap-6 font-sans">
    ...
    <div className="flex justify-between items-center">
      <div>
        <h1 className="text-2xl font-bold text-primary">Pipeline de Ventas</h1>
    ...
    <div className="grid grid-cols-3 gap-4">
      <div className="bg-white p-4 rounded-xl shadow-md border border-gray-100">
        ...
        <p className="text-2xl font-bold text-gray-800">{filteredOps.length}</p>
      </div>

      <div className="bg-white p-4 rounded-xl shadow-md border border-gray-100">
        <p className="text-xs text-gray-500">Monto total filtrado</p>
        <p className="text-2xl font-bold text-gray-800">
          {totalMonto.toLocaleString("es-CL", { minimumFractionDigits: 0 })}
        </p>
      </div>
...
<div className="overflow-x-auto pb-4">
  ...
  >
    ...
    {/* ⭐ DragOverlay — evita que la tarjeta desaparezca */}
    <DragOverlay>
      {activeId ? (
        <OpportunityCard
